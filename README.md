...
이번 버전은 샘플 UI MVP이며 실제 AI 처리 기능은 포함하지 않습니다. 메뉴 이동, 해시 주소, 작업 상태 필터, 결과 미리보기, 보기 설정, 샘플 표시 설정과 방문 집계만 동작합니다. `AI 회의록`과 `AI 쇼츠 스튜디오`는 실제 기능을 연결할 설명 화면만 제공합니다.
...
현재 화면에서 홈, AI 도구 설명, 작업·결과물·파일, 사용량, 멤버십, 설정·도움말을 확인할 수 있습니다. 기본 샘플 데이터는 설정에서 숨길 수 있으며, 멤버십 연결 전 상태에서는 계정 권한, 업로드, 생성, 외부 API 호출, 결제를 요청하지 않습니다. 공개 환경에서 허용하는 외부 통신은 Builders Lounge 방문 집계 한 건뿐입니다.
