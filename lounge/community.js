...
function copyToClipboard(text, button, successText = "복사됨") {
  ...
  const done = () => { button.textContent = successText; window.setTimeout(() => { button.textContent = original; }, 1500); };
  if (navigator.clipboard?.writeText) navigator.clipboard.writeText(text).then(done).catch(() => { button.textContent = "원문을 선택해 복사해 주세요"; });
